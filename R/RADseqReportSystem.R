#source('./R/init.R') #Loading external setting files
...
if(is.null(in_stacksout)){
    in_stacksout <-  as.character(args[2])
}

if(is.null(in_title)){
    in_title <-  as.character(args[3])
}
...
if(is.null(html_desc)){
    html_desc <-  as.character(args[5])
}
...
for(i in 2:(OL-1)){
	...
	if (depthdetect > 0) {
		...
		wSNPd30 <- nrow(subset(wSNPdepth["depth"], wSNPdepth["depth"]>=30))
		wSNPd100 <- nrow(subset(wSNPdepth["depth"], wSNPdepth["depth"]>=100))
		dL1 <- hwrite(c("Locus number","all","with SNP"), col.width=c('120px','80px','90px'), col.style=c('text-align:left','text-align:right','text-align:right'), bgcolor='#aaffaa', border=0, cellspacing=0)
		dL2 <- hwrite(c("all",alld,wSNPd), col.width=c('120px','80px','90px'), col.style=c('text-align:left','text-align:right','text-align:right'), bgcolor=c('#aaffaa',"","",""), border=0, cellspacing=0)
		dL3 <- hwrite(c("depth >= 3",alld3,wSNPd3), col.width=c('120px','80px','90px'), col.style=c('text-align:left','text-align:right','text-align:right'), bgcolor=c('#aaffaa',"","",""), border=0, cellspacing=0)
		dL4 <- hwrite(c("depth >= 10",alld10,wSNPd10), col.width=c('120px','80px','90px'), col.style=c('text-align:left','text-align:right','text-align:right'), bgcolor=c('#aaffaa',"","",""), border=0, cellspacing=0)
		dL5 <- hwrite(c("depth >= 30",alld30,wSNPd30), col.width=c('120px','80px','90px'), col.style=c('text-align:left','text-align:right','text-align:right'), bgcolor=c('#aaffaa',"","",""), border=0, cellspacing=0)
...
plot(contig~matching.samples, pallmatch, type="l", xaxt="n", ylim=c(0, max(pallmatch$contig)), col="steelblue2", xlab="Matching sample (%)",ylab="Number of loci", main="Locus number", cex.main=1.4, cex.lab=1.4, cex.axis=1.2)
...
legend(43, max(pallmatch$contig), paste(c("all","loci with multiple SNPs","loci with 1SNP 2 alleles" )), col=c("steelblue2","orange","yellow"), pch=15, cex=1.2, bg="white")
dev.off()
#setwd("../")
...
cL1 <- hwrite(c("loci","all","1SNP 2 alleles","with 1 and multiple SNP(s)"), col.width=c('180px','90px','120px','100px'), col.style=c('text-align:left','text-align:right','text-align:right','text-align:right'), bgcolor='#aaffaa', border=0, cellspacing=0)
...
#Changing status for "finish", after all tasks had done
if(!is.null(finish_file)){
