...
inR <- as.character(args[1])        #ログファイル
...
#  in_email <- "mihara@dynacom.co.jp"
  in_title <- "Test Mihara"
  in_dname <- "dname"
  in_enzyme1 <- "AA1"
  in_enzyme2 <- "BB2"
  in_polymerase <- "CC3"
...
ReadNumberHist <- ggplot(SampleSummary, aes(x = log10read))+geom_histogram(binwidth=0.5, fill="seagreen4")+ xlim(0,8)+ labs(title=paste("Histogram of ",tolower(wr_rnum),"\nper sample",sep=""), x=bquote(paste(Log[10]," ",.(tolower(wr_rnum)),sep="")), y="Frequency")+ theme(plot.title = element_text(size = ggmain.size),axis.title.x = element_text(size=gglab.size),axis.title.y = element_text(size=gglab.size))
...
i <- 1
	...
	if (depthdetect > 0) {
		...
		title(xlab="Depth", ylab="Frequency", main="Histogram of depth (frequency < 500)", cex.lab=1.4, cex.main=1.4)
		dev.off()
...
for(i in 1:OL){
	...
	if (depthdetect > 0) {
		...
		if(length(depth3$upper100) > 0){
			png1 <- paste(out_img_dir, "/", h, "_depth3.png", sep="")
			d3 <- ggplot(depth3, aes(x=upper100))+geom_histogram(binwidth=5, fill="seagreen4")+ scale_x_continuous(limits=c(0,105), labels=c("0", "25", "50", "75", ">100"))+labs(title="Histogram of depth (depth >= 3)", x="Depth", y="Frequency")+ theme(plot.title = element_text(size = ggmain.size),axis.title.x = element_text(size=gglab.size),axis.title.y = element_text(size=gglab.size))
		...
		if(length(y100) != 0 ){
			...
			ip <- gb2+ geom_hex()+ scale_x_continuous(limits = c(0, 1), breaks=seq(0,1,0.2))+ scale_fill_continuous(trans = "log10")+ scale_y_continuous(limits = c(0, 100), labels = c("0", "25", "50", "75", ">100"))+labs(title="Major allele depth vs total depth", x="fraction of major allele in the contigs (<1)", y="Depth")+ theme(legend.margin = unit(0, "cm"))
...
legend(43, max(pallmatch$contig), paste(c("all","contigs with SNPs","contigs with 1SNP 2 alleles" )), col=c("steelblue2","orange","yellow"), pch=15, cex=1.2, bg="white")
dev.off()
#setwd("../")
...
readnumstacks <- ggplot(depthsummary, aes(x = lreadnumstacks))+geom_histogram(binwidth=0.5, fill="seagreen4")+ xlim(0,8)+ labs(title=paste("Histogram of ",tolower(wr_rnum),"\nused in stacks",sep=""), x=bquote(paste(Log[10]," ",.(tolower(wr_rnum)),sep="")), y="Frequency")+ theme(plot.title = element_text(size = ggmain.size),axis.title.x = element_text(size=gglab.size),axis.title.y = element_text(size=gglab.size))
ggsave(file = paste(out_img_dir,"readnumstacks.png",sep="/"), plot = readnumstacks, dpi = 100, width = 4, height = 4)
meddepthstacks <- ggplot(depthsummary, aes(x = meddepth))+geom_histogram(binwidth=1, fill="seagreen4")+ labs(title="Median depth of samples", x="Median depth", y="Frequency")+ theme(plot.title = element_text(size = ggmain.size),axis.title.x = element_text(size=gglab.size),axis.title.y = element_text(size=gglab.size))
ggsave(file = paste(out_img_dir,"mediandepthHist.png",sep="/"), plot = meddepthstacks, dpi = 100, width = 4, height = 4)
maxdepthstacks <- ggplot(depthsummary, aes(x = maxdepth))+geom_histogram(binwidth=50, fill="seagreen4")+ labs(title="Max depth of samples", x="Max depth", y="Frequency")+ theme(plot.title = element_text(size = ggmain.size),axis.title.x = element_text(size=gglab.size),axis.title.y = element_text(size=gglab.size))
...
colnames(depthlist) <- c("average depth", "contigs", paste("number of used ",tolower(wr_rd),sep=""), paste("fraction of used ",tolower(wr_rd)," (%)",sep=""))
j <- 1
for(i in c(0,1,3,10,30,100)){
	contigs <- subset(catdl, catdl$average.depth > i)
	ad <- paste(">", i, sep="")
	depthlist[j, "average depth"] <- ad
	depthlist[j, "contigs"] <- nrow(contigs)
	depthlist[j, paste("number of used ",tolower(wr_rd),sep="")] <- sum(contigs$total.depth)
	depthlist[j, paste("fraction of used ",tolower(wr_rd)," (%)",sep="")] <- floor(sum(contigs$total.depth)*100/sum(catdl$total.depth))
	j <- j+1
}

#setwd(reportID)
catdl$laverage <- log10(catdl$average.depth)
adepth <- ggplot(catdl, aes(x = laverage))+geom_histogram(binwidth=0.2, fill="seagreen4")+ xlim(-3,5)+ labs(title="Average depth of locus", x=bquote(paste(Log[10]," (average depth)",sep="")), y="Frequency")+ theme(plot.title = element_text(size = ggmain.size),axis.title.x = element_text(size=gglab.size),axis.title.y = element_text(size=gglab.size))
...
title(xlab=bquote(paste(Log[10]," (average depth)",sep="")), ylab="Frequency", main="Average depth of locus (frequency < 3000)", cex.lab=1.4, cex.main=1.4)
dev.off()
...
cL1 <- hwrite(c("contigs","all","1SNP 2 alleles","with SNPs"), col.width=c('180px','90px','120px','100px'), col.style=c('text-align:left','text-align:right','text-align:right','text-align:right'), bgcolor='#aaffaa', border=0, cellspacing=0)
...
LSGd <- hwrite(c("General information about the library", "This section contains information such as the date of the Stack analysis, number of samples, and frequency distributions of read number and quality value."), dim=c(2,1), col.names=FALSE, border=0)
...
LSRd <- hwrite(c("Information about the RAD-Seq library", "This section contains information such as Stacks parameters used, the number of contigs in each percentage of matching samples, and frequency distributions of read number, depth, and other metrics."), dim=c(2,1), col.names=FALSE, border=0)
...
SSd <- hwrite(c("Information about each sample", "General: read number, mean quality value, and their relationship.","RAD-Seq: Stacks read number, frequency distribution of depths, the relationship between contig number and depth, and the relationship between depth and the depth fraction of the major allele."), dim=c(3,1), col.names=FALSE, border=0)
...
sampletable[paste("Fraction of ",tolower(wr_rd)," used in stacks (%)",sep="")] <- floor(sampletable[3]*100/sampletable[2])
